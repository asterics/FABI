...
// --------------------------------------------------------------
// File : main.c
// Author : Benjamin Aigner, Peter Glößl
...
int main (int argc, char *argv[])
{
    ...
    // Start Timer for periodic comlist
    timer = g_timeout_add_seconds (5, TimerCallback, NULL);
...
//    while ((token = strsep((char**)&comlist, ";")) != NULL)
    ...
    //Drop Down "COM Port"
    comboCOM = gtk_combo_box_text_new();
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(comboCOM), NULL, (gchar*)""); // create a first empty item
...
void fillComboCOM(void)
{
        ...
        while (index > 0)
        {
            gtk_combo_box_text_remove (GTK_COMBO_BOX_TEXT(comboCOM), 1);    // delete first item
            index--;
        }
//        gtk_combo_box_text_remove_all (GTK_COMBO_BOX_TEXT(comboCOM));     // causes an error
...
//       while ((token = strsep((char**)&comlist, ";")) != NULL)
        {
            if(strcmp(token,"") != 0)
            {
                gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(comboCOM), NULL, (gchar*)token);   // write one item at the end of the list
                index++;
//                printf("activeCOM: %s\nToken: %s\n", activeCOM, token);
                if (activeCOM != NULL)
                {
                   if ((strcmp(activeCOM, token)) == 0)
                    {
                     gtk_combo_box_set_active (GTK_COMBO_BOX(comboCOM), index);
                    }
                }
