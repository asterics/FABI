...
uint16_t unicode_to_keycode(uint16_t cpoint, uint8_t locale)
{
		...
		if (cpoint == 10) 
    {
      /*#ifdef DEBUG_OUTPUT_KEYS
        Serial.printf("cpoint %d, locale %d, mask %d, key %d\n",cpoint,locale,keycodes_masks[locale][3],KEY_ENTER);
      #endif*/
    ...
    {
      /*#ifdef DEBUG_OUTPUT_KEYS
        Serial.printf("cpoint %d, locale %d, mask %d, key %d\n",cpoint,locale,keycodes_masks[locale][3],KEY_TAB);
      #endif*/
  ...
  {
    /*#ifdef DEBUG_OUTPUT_KEYS
      Serial.printf("ASCII lookup");
      Serial.printf("cpoint %d, locale %d, mask --, key %d\n",cpoint,locale,keycodes_ascii[locale][cpoint - 0x20]);
    #endif*/
    return keycodes_ascii[locale][cpoint - 0x20];
  }
	if (cpoint <= 0xA0) 
  {
    //Serial.printf("unkown cpoint\n");
    return 0;
  }
	if (cpoint < 0x100)
  {
    ...
    {
      //Serial.printf("locale is LAYOUT_US_ENGLISH, no unicode available\n");
      return 0;
    } else {
      /*#ifdef DEBUG_OUTPUT_KEYS
        Serial.printf("cpoint %d, locale %d, mask --, key %d\n",cpoint,locale,keycodes_iso_8859_1[locale-1][cpoint-0xA0]);
      #endif*/
      return keycodes_iso_8859_1[locale-1][cpoint-0xA0];
    }
  }
	//TODO: do UNICODE_EXTRA characters here....
  //Serial.printf("nothing applies in unicode_to_keycode\n");
	return 0;
}
...
uint16_t deadkey_to_keycode(uint16_t keycode, uint8_t locale)
{
  ...
  /*#ifdef DEBUG_OUTPUT_KEYS
    Serial.printf("deadkeys: applying mask 0x%X, result: %d\n", keycodes_masks[locale][2], keycode);
  #endif*/
  for(uint8_t i = 0; i<sizeof(keycodes_deadkey_bits[locale]); i++)
  {
    ...
    {
      /*#ifdef DEBUG_OUTPUT_KEYS
        Serial.printf("deadkey found, index: %d, deadkey: %d\n",i,keycodes_deadkey[locale][i]);
      #endif*/
      return keycodes_deadkey[locale][i];
    }
  }
  //Serial.printf("no deadkey\n");
	return 0;
}
...
uint8_t get_hid_country_code(uint8_t locale)
{
  switch(locale)
  {
    ...
    default:
      //Serial.printf(,"no hid country code found for locale %d",locale);
      return 0;
  }
}
