...
const struct atCommandType atCommands[] PROGMEM = {
  // PROGMEM, meaning that this array will be saved in the Flash memory of the MC, instead of in the RAM.
  /***** mouse *****/
  { "ID", PARTYPE_NONE },
  { "BM", PARTYPE_UINT },  // PARTYPE is the parameter of the command.
  { "CL", PARTYPE_NONE },
  { "CR", PARTYPE_NONE },
  { "CM", PARTYPE_NONE },
  { "CD", PARTYPE_NONE },
...
void performCommand(uint8_t cmd, int16_t par1, char *keystring, int8_t periodicMouseMovement) {
  ...
  if (actButton != 0)  // if last command was BM (set buttonmode): store current command for this button !!
  {
    ...
    Serial.print("got new mode for button ");
    Serial.print(actButton);
    Serial.print(":");
    Serial.print(cmd);
    Serial.print(",");
    Serial.print(par1);
  ...
  switch (cmd) {
    ...
    case CMD_BM:
      release_all();
#ifdef DEBUG_OUTPUT_FULL
      Serial.print("set mode for button ");
      Serial.println(par1);
#endif
    ...
    case CMD_HR:  // HR, Hold right mouse button.
      mousePress(MOUSE_RIGHT);
      break;
    ...
    case CMD_SA:  // SA, Save slotSettings.
      release_all();
      if (keystring) {
    ...
    case CMD_RS:                                                                 // RS, Reset Fabi settings and restore default configurations.
    ...
    case CMD_RE:  // RE, Reboot.
      watchdog_reboot(0, 0, 10);
      while (1) { continue; }  // AK: Why does this exist, if it always is true and just exits the loop afterwards.
      break;
    case CMD_NC:  // NC,, No command.
      break;
    case CMD_SR:  // SR, start reporting raw values.
      reportRawValues = 1;
      break;
    case CMD_ER:
      reportRawValues = 0;
